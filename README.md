...
```
Примечания:
1. Каталог под репозиторий будет создан, если его не существует.
...
1. `-a` ака `--all` повелевает перенести в staging area модификации отслеживаемых файлах, чтобы они тоже попали в коммит.
