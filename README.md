...
Для каждого нового коммита запускается pipeline, в pipeline может быть несколько стадий (stage), а в каждой стадии — несколько задач (job).

Runner — это процесс, который выполняет задачи, поступающие от GitLab.
...
Например, `CI_COMMIT_SHORT_SHA`, `CI_JOB_ID`, `CI_PIPELINE_ID` и т.д.
Можно задавать собственные переменные, например:
```
variables:
  LANG: en_US.UTF-8
```
